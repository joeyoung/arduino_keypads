...
int Keypad_tw::pin_read(byte pinNum) {
	...
	USI_TWI::requestFrom((int)twaddr, (int)twwidth);
	word pinVal = USI_TWI::receive( );
	if (twwidth > 1) {
		pinVal |= USI_TWI::receive( ) << 8;
	} 
	pinVal &= mask;
	if( pinVal == mask ) {
		return 1;
...
void Keypad_tw::port_write( word twportval ) {
	USI_TWI::beginTransmission((int)twaddr);
	USI_TWI::send( twportval & 0x00FF);
	if (twwidth > 1) {
		USI_TWI::send( twportval >> 8 );
	}
	USI_TWI::endTransmission();
	pinState = twportval;
} // port_write( )

word Keypad_tw::pinState_set( ) {
	USI_TWI::requestFrom( (int)twaddr, (int)twwidth );
	pinState = USI_TWI::receive( );
	if (twwidth > 1) {
		pinState |= USI_TWI::receive( ) << 8;
	}
	return pinState;
} // set_pinState( )
